...
**Hyprland, Sway and niri also install `noctalia-shell`.** A compositor is a
...
outright**, niri is happy with `foot`. **Hyprland also greets you with a
warning across the top of the screen** on the first login — it writes its own
config into your home and says so until you edit it. That banner is Hyprland's,
not a failure, and no NixOS option removes it: delete the line holding
`autogenerated = true` from that file. SUPER+Q opens the terminal, SUPER+R the
runner. Sway already brings foot and wmenu, and

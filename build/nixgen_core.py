...
# channel: {channel}   generated: {stamp}{build}
#
...
def render_lines(entries, indent="  "):
    """The attribute lines of a module, with no header and no braces.

    `render_module` is this plus a header and a closing brace. The starter
    needs the lines on their own: what comes out of somebody's
    configuration.nix is carried into the configuration.nix it writes, and
    that file already has its own braces.
    """
    out = []
    for e in sorted(entries, key=lambda x: x.get("path") or ".".join(x["segments"])):
        ...
        out.append(f"{indent}{render_path(segs)} = {expr};{tail}\n")
    return "".join(out)


# A build id is `2026-08-11m` — a date and a letter, nothing else. It is
# checked rather than trusted because it is written into a file the user keeps,
# and it arrives from the client.
_BUILD_ID = re.compile(r"^[0-9]{4}-[0-9]{2}-[0-9]{2}[a-z]?$")


def render_module(entries, channel, stamp, build=None):
    ...
    `build` names the nixgen that wrote the file. It is in the header because a
    ...
    fix is what you ran" are different facts, and only the file can say which.
    """
    tag = f"   nixgen: {build}" if build and _BUILD_ID.match(build) else ""
    return (HEADER.format(channel=channel, stamp=stamp, build=tag)
            + render_lines(entries) + "}\n")
